...
(out / 'grove-builds.json').write_text(json.dumps(builds, indent=2) + '\n')
counts = []
for source, label in [('test', 'editmode'), ('playtest', 'playmode')]:
    ...
    shutil.copyfile(path, out / f'grove-{label}.xml')
metrics = {}
for quality in ['low', 'high']:
    ...
    shutil.copyfile(path, out / f'grove-{quality}.json')
    ...
    for name in ['01-entrance', '02-trail-and-doe', '03-fern-trail', '04-stump-and-floor']:
        assert (capture / (name + '.png')).read_bytes()[:8] == b'\x89PNG\r\n\x1a\n'
    shutil.copyfile(capture / 'player.log', out / f'grove-{quality}-capture.txt')
...
shutil.copyfile(smoke / 'builder.json', out / 'grove-shared-ranch.json')
shutil.copyfile(root / 'Logs/grove-multiplayer.txt', out / 'grove-multiplayer.txt')
for quality, source, target in [('high', '02-trail-and-doe', 'grove-high'), ('high', '01-entrance', 'grove-entrance'), ('high', '04-stump-and-floor', 'grove-detail'), ('low', '02-trail-and-doe', 'grove-low')]:
    shutil.copyfile(root / f'Logs/grove-capture/{quality}/{source}.png', root / f'docs/benchmarks/{target}.png')
subprocess.run([sys.executable, str(root / '.agents/tools/source-manifest.py'), str(out / 'grove-source-files.json')], check=True)
