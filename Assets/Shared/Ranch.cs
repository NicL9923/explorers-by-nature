...
namespace ExplorersByNature.Shared
{
    ...
    {
        public int schema = 1, revision, nextId = 1, milk, eggs;
    ...
    {
        public static readonly string[] Kinds = { "foundation", "wall", "door", "roof", "fence", "flower" };
        ...
        public Ranch(string path, Func<object,string> encode, Func<string,RanchState> decode, Func<float,float,float> ground)
        {
            ...
            if (state == null || state.schema != 1 || state.pieces == null || state.pieces.Count > 4000 || state.revision<0 || state.milk<0 || state.eggs<0 || state.cowReady<0 || state.henReady<0)
        ...
        public string Apply(Request r, Visitor player, long now)
        {
            ...
            if (r.action == "milk" || r.action == "eggs")
            {
            ...
            else if (r.action == "place" || r.action == "move" || r.action == "remove")
            {
                ...
                else
                {
                    ...
                    if (r.action == "move" && existing.kind == "foundation" && next.pieces.Any(p=>p.id!=existing.id && p.x==existing.x && p.z==existing.z && (p.kind=="wall" || p.kind=="door" || p.kind=="roof"))) return "Move the walls and roof first.";
                    if (existing != null) next.pieces.Remove(existing);
                    bool edge = r.kind=="wall" || r.kind=="door" || r.kind=="fence";
                    if (next.pieces.Any(p=>p.x==r.x && p.z==r.z && ((edge && (p.kind=="wall" || p.kind=="door" || p.kind=="fence") && p.turn==r.turn) || (!edge && p.kind==r.kind)))) return "There is already a piece in that slot.";
                    if ((r.kind=="wall" || r.kind=="door" || r.kind=="roof") && !next.pieces.Any(p=>p.x==r.x && p.z==r.z && p.kind=="foundation")) return "Place a foundation first.";
                    if (next.pieces.Count >= 4000) return "This valley has reached its 4,000-piece limit.";
