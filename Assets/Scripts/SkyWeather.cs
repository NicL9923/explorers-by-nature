...
namespace ExplorersByNature
{
    public sealed class SkyWeather : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            RenderSettings.ambientSkyColor=Color.Lerp(new Color(.38f,.45f,.58f),new Color(.75f,.81f,.86f),Daylight);
            RenderSettings.ambientEquatorColor=Color.Lerp(new Color(.28f,.34f,.40f),new Color(.57f,.62f,.46f),Daylight);
            RenderSettings.ambientGroundColor=Color.Lerp(new Color(.20f,.24f,.30f),new Color(.32f,.36f,.23f),Daylight);
            ...
            if(sun!=null)
            {
                float elevation=15+Mathf.Sin(phase*Mathf.PI)*45;
                sun.transform.rotation=Quaternion.Euler(elevation,-32,0);
                sun.intensity=Mathf.Lerp(.28f,1.45f,Daylight)*(1-RainAmount*.3f);
                sun.color=Color.Lerp(new Color(.72f,.80f,1),Color.Lerp(new Color(1,.75f,.48f),new Color(1,.96f,.86f),Daylight),Mathf.Clamp01(Daylight*3));
